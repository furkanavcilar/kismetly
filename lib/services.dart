...
class PlanetInfo {
  const PlanetInfo(
      {required this.name, required this.headline, required this.detail});
  final String name;
...
class AstroService {
  ...
  static CompatibilityReport compatibility(String first, String second) {
    ...
    final score =
        (baseScore + (elementA == elementB ? 0.08 : 0)).clamp(0.0, 1.0);
  ...
  static Future<String> fetchDailyQuote() async {
    ...
    try {
      final r = await http
          .get(
            Uri.parse(
                'https://api.quotable.io/random?tags=inspirational|wisdom'),
          )
  ...
  static Future<HoroscopeBundle> fetchHoroscopeBundle(String sign) async {
    ...
    Future<String> load(String period) async {
      try {
        final u = Uri.parse(
            'https://horoscope-app-api.vercel.app/api/v1/get-horoscope/$period?sign=$en');
    ...
    final res =
        await Future.wait([load('daily'), load('monthly'), load('yearly')]);
    ...
    if (list.isEmpty) {
      return const PlanetarySnapshot([
        PlanetInfo(
            ...
            detail: 'Ortalama sıcaklık: -63°C'),
        PlanetInfo(
            name: 'Venüs',
            headline: 'İç ısı ve tutku',
            detail: 'Ortalama sıcaklık: 464°C'),
      ]);
    }
  ...
  static Future<WeatherSnapshot?> fetchWeather({
    ...
    try {
      ...
      return WeatherSnapshot(
        ...
        apparentTemperature:
            (cur['apparent_temperature'] as num?)?.toDouble() ?? 0,
        ...
        minTemp: ((daily['temperature_2m_min'] as List?)?.first as num?)
                ?.toDouble() ??
            0,
        maxTemp: ((daily['temperature_2m_max'] as List?)?.first as num?)
                ?.toDouble() ??
            0,
        observationTime:
            DateTime.tryParse((cur['time'] as String?) ?? '')?.toLocal() ??
                DateTime.now(),
      );
    } catch (e) {
